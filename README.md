...
Отпечаток сертификата, которым подписаны release-сборки из этого репозитория
(проверить: `apksigner verify --print-certs app-universal-release.apk`):

SHA-256:
`50:DF:1D:DA:22:81:E8:3F:C5:3A:DF:DB:A9:63:56:78:0B:F0:4B:36:EC:27:8B:61:03:A3:B5:AD:EA:4C:AC:67`

> Debug-сборки подписаны стандартным debug-ключом Android, у них другой отпечаток.
> Обновления приложения работают только при совпадении подписи: если вы поставили
> debug-APK, то release-APK поверх него не установится — сначала удалите приложение
> (настройки сохраните экспортом).
